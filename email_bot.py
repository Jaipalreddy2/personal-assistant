...
def fetch_emails_since_dt(since_dt):
    """Fetch emails received strictly after since_dt (timezone-aware datetime)."""
    mail = imaplib.IMAP4_SSL("imap.gmail.com")
    mail.login(GMAIL_ADDRESS, GMAIL_APP_PASS)
    mail.select("inbox")
    ...
    for mid in msg_ids[0].split():
        ...
        emails.append({"subject": subject, "from": sender, "date": date_str, "body": body[:1500]})
